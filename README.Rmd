...
* To set the previous folder in -c, you need to follow the following rule:
  ...
  You can find "*grouped_IS" files by using /home/ayan/Aimin/Seagate/ISseqOutput/Oct22New/CutAdapt/filterNo/db/*grouped_IS
  
  The previous folder name you need to give is /home/ayan/Aimin/Seagate/ISseqOutput/Oct22New
...
You can find the ouput results in the following folders:
```
...
Note, IS-Seq has flexibilities to let users define input file and output file path to NAS(Network-Attached Storage). To do this on Linux machine , user need to mount NAS to Linux firstly. Here is the steps to mount NAS to Linux:
...
You can find UMI based outputs in the following resutls:
```
...
```
You can also check the number of reads that have same UMI in the following folder:
```
/home/ayan/Aimin/IS-Seq/ISseqOutput/Mar06/UmiBased/UmiCluster/UmiBarPlots
```
e.g. this following figure shows the distribution of reads in top 100 UMIs before alignment
![umi](doc/allFiles_UMI.png){width=80%}
We also include the following Figure to demonstarte the distribution of reads in top 100 UMIs after paired-end alignment:
![umiAfterAlignment](doc/all_files_UMI_after_alignment.png){width=80%}
...
You can find fragment based outputs in the following folders:
```
...
* Command line arguments:
  ...
  *output_dir: the directory for writing output_file to
  ...
  *inputvectorFq: the reads from FASTQ files
